...
- **Proxy pool rotation** (`grok.py`): loads `keys/good_proxies.txt` (or `GROK_PROXY_LIST` env) and rotates to a fresh proxy per registration attempt. **Accepts both `http` and `socks5h` proxies** (Cloudflare passes SOCKS5 free proxies that 403 on HTTP proxy IP classes). xAI throttles validation-code emails **per IP**, so one proxy per account bypasses the limit.
- **mail.tm inbox provider** (`email_service.py`): free API-based temp inbox, no browser needed. Critical because xAI throttles per **Gmail base account** (all `+alias` addresses share the mailbox) after ~5-6 code sends. Each mail.tm inbox is a separate mailbox with no shared cap.
...
- Loads `auths/xai-*.json`, round-robins across accounts (async, concurrent)
- **Background refresh loop** — keeps every account's OAuth token fresh (refreshes 5 min before expiry, checks every 60s)
- **RR failover** — on any upstream error (429/401/5xx/timeout) marks the account in cooldown (5 min) and retries the next one
- 90s per-read timeout (under Cloudflare's 100s tunnel ceiling, so a stalled upstream fails over instead of 524ing)
- Exposes `/v1/models`, `/v1/chat/completions` (streaming), `/v1/images/generations`
- **reasoning_effort passthrough** — flattens opencode's nested `body` overlay (e.g. `body.reasoning_effort`) into the request so xAI's top-level `reasoning_effort` works (see "opencode variants" below)
...
### OpenCode integration (reasoning variants)

The proxy is OpenAI-compatible, so it plugs into opencode as a custom provider. Because the proxy flattens the `body` overlay, per-model reasoning variants work:
...
Select via `opencode --model grok-farm/grok-4.6 --variant high`. **Note:** on the consumer free tier, reasoning tokens count against the ~10 msgs/2h per-account quota, and `high` burns 4-6x the quota of a normal request. Fine for interactive use, not for 24/7 agents.
