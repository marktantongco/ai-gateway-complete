...
class GmailIMAPClient:
    ...
    def fetch_first_email(self) -> Optional[str]:
        ...
        try:
            self._connect()
            # 只按收件人精确匹配本别名，绝不退回"最新x.ai邮件"(会抓到别的别名的旧码)
            search_criteria = f'(FROM "x.ai" TO "{self.email}")'
            _, data = self._imap.uid("SEARCH", None, search_criteria)
            ...
            raw = msg_data[0][1]
            msg = _email_mod.message_from_bytes(raw)
            # 主题优先返回，验证码在主题里是干净的 "594-475" 格式；
            # 正文是 HTML，含 CSS 噪音(如 333333)会污染正则提取
            parts = [f"Subject: {msg.get('Subject') or ''}"]
            body = ""
            if msg.is_multipart():
                for part in msg.walk():
