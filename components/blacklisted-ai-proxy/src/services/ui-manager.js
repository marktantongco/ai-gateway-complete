...
export async function handleUIApiRequests(method, pathParam, req, res, currentConfig, providerPoolManager) {
    ...
    // Import Grok/X session cookies (from browser extension or manual paste)
    if (method === 'POST' && pathParam === '/api/auth/import-session') {
        const { handleSessionImport } = await import('../auth/session-import.js');
        return await handleSessionImport(req, res);
    }
