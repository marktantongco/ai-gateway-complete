...
let allConfigs = []; // stores all config data
let filteredConfigs = []; // stores filtered config data
let isLoadingConfigs = false; // prevents duplicate config loading
...
function searchConfigs(searchTerm = '', statusFilter = '', providerFilter = '') {
    // ensure searchTerm is a string, prevent event objects etc. from being passed in
    ...
    filteredConfigs = allConfigs.filter(config => {
        ...
        if (providerFilter) {
            ...
            if (providerFilter === 'other') {
                // "other/unrecognized" option: match configs with no recognized provider
                matchesProvider = providerInfo === null;
            } else {
                // match specific provider type
...
function createConfigItemElement(config, index) {
    ...
    // generate expiration time HTML
    let expirationHtml = '';
    if (config.expiresAt) {
        // prefer server-calculated remaining time to avoid client clock sync issues
        ...
        let timeStr = '';
        if (isExpired) {
            timeStr = `<span class="expiration-tag expired">${t('upload.expiration.expired') || 'Expired'}</span>`;
        } else {
            ...
            let duration = '';
            if (days > 0) duration += `${days}${t('common.date.days') || 'd'}`;
            if (hours > 0 || days > 0) duration += `${hours}${t('common.date.hours') || 'h'}`;
            duration += `${minutes}${t('common.date.minutes') || 'm'}`;
            
            timeStr = `<span class="expiration-tag ${remainingSeconds < 3600 * 24 ? 'warning' : 'healthy'}">
                <i class="fas fa-clock"></i> ${duration} ${t('upload.expiration.remaining') || 'remaining'}
            </span>`;
        }
        
        expirationHtml = `<div class="config-expiration-info">
            <span class="meta-item" title="${t('upload.detail.expiresAt') || 'Expiration time'}: ${formatDate(config.expiresAt)}">
                ${timeStr}
            </span>
        </div>`;
    }

    // get linked node brief info
    let linkedNodesInfo = '';
    if (config.isUsed && config.usageInfo && config.usageInfo.usageDetails) {
        const details = config.usageInfo.usageDetails;
        
        // collect node info and status
        const nodes = details.map(d => {
            let name = '';
            let isPool = false;
            if (d.type === 'Provider Pool' || d.type === 'Provider Pool') {
                isPool = true;
                if (d.nodeName) name = d.nodeName;
                else if (d.uuid) name = d.uuid.substring(0, 8);
                else name = d.location;
            } else if (d.type === 'Main Config' || d.type === 'Main Config') {
        ...
        if (nodes.length > 0) {
            // Deduplicate while preserving status info (if multiple nodes have the same name, status may differ; here deduplicate by name, keeping the first)
            const uniqueNodes = [];
    ...
    const quickLinkBtnHtml = canQuickLink ?
        `<button class="btn-quick-link-main" data-path="${config.path}" title="Quick link to ${providerInfo.displayName}">
    ...
    const forceExpireBtnHtml = canForceExpire ?
        `<button class="btn-small btn-force-expire" data-path="${config.path}">
            <i class="fas fa-sync"></i> <span data-i18n="upload.action.forceExpire">${t('upload.action.forceExpire') || 'Force Refresh'}</span>
        </button>` : '';
    ...
    item.innerHTML = `
        ...
        <div class="config-item-details">
            <div class="config-details-grid">
                ...
                <div class="config-detail-item">
                    <div class="config-detail-label" data-i18n="upload.detail.status">Status</div>
    ...
    if (pathValueEl) {
        ...
        pathValueEl.title = t('models.clickToCopy') || 'Click to copy';
    }
