...
OpenClaw で BlacklistedAPI を使用するためのクイック設定ガイド。

---

## 前提条件

1. BlacklistedAPI サービスを起動
...
# 特定のモデルでチャット
openclaw chat --model blacklistedapi/gemini-3-flash-preview "あなたの質問"
```
...
**Q: 接続に失敗しますか？**
- BlacklistedAPI サービスが実行中であることを確認
...
**Q: モデルが利用できない？**
- BlacklistedAPI Web UI でプロバイダーが設定されているか確認
- `openclaw gateway restart` を実行してゲートウェイを再起動
- `openclaw models list` を実行してモデルリストを確認

---

詳細については、[BlacklistedAPI ドキュメント](../README-JA.md) を参照してください
